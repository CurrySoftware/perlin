...
use field::{FieldDefinition, Field};
...
pub trait IndexContainer<T: Hash + Eq> {
    ...
    fn index_field(&mut self, DocId, Field<T>);

    fn query_field(&self, &Field<T>) -> Vec<Posting>;
}
...
mod tests {
    ...
    impl IndexContainer<String> for TestContainer {
        ...
        fn index_field(&mut self, doc_id: DocId, field: Field<String>) {
            if let Some(index) = self.text_fields.get_mut(&(field.0).0) {
                index.index_document(field.1.split_whitespace().map(|s| s.to_string()),
                                     Some(doc_id));
            }
        }

        fn query_field(&self, query: &Field<String>) -> Vec<Posting> {
            if let Some(index) = self.text_fields.get(&(query.0).0) {
                return index.query_atom(&query.1);
            }
            panic!();
        }
    }

    impl<'a> IndexContainer<&'a str> for TestContainer {
        fn manage_index(&mut self, def: FieldDefinition, index: Index<&'a str>) {
            unimplemented!();
        }

        fn index_field(&mut self, doc_id: DocId, field: Field<&'a str>) {
            if let Some(index) = self.text_fields.get_mut(&(field.0).0) {
                index.index_document(field.1.split_whitespace().map(|s| s.to_string()),
                                     Some(doc_id));
            }
        ...
        fn query_field(&self, query: &Field<&'a str>) -> Vec<Posting> {
            if let Some(index) = self.text_fields.get(&(query.0).0) {
                return index.query_atom(&query.1.to_string());
            }
            panic!();
        }
    }
    ...
    impl IndexContainer<u64> for TestContainer {
        ...
        fn index_field(&mut self, doc_id: DocId, field: Field<u64>) {
            if let Some(index) = self.num_fields.get_mut(&(field.0).0) {
                index.index_document(vec![field.1].into_iter(), Some(doc_id));
            }
        }

        fn query_field(&self, query: &Field<u64>) -> Vec<Posting> {
            if let Some(index) = self.num_fields.get(&(query.0).0) {
                return index.query_atom(&query.1);
            }
            panic!();
        }
    ...
    fn multiple_fieldtypes() {
        ...
        index.index_field(DocId(0), Field(text_field1,
        ...
        index.index_field(DocId(1), Field(text_field1,
        ...
        index.index_field(DocId(2), Field(text_field1,
        ...
        assert_eq!(index.query_field(&Field(text_field0, "Moon")),
                   vec![DocId(2)]);
        assert_eq!(index.query_field(&Field(num_field2, 4)),
                   vec![DocId(0)]);
        assert_eq!(index.query_field(&Field(num_field3, 1)),
                   vec![DocId(1)]);
